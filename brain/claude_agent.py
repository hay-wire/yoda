...
PROMPT_TEMPLATE = """You are extracting work items from raw messages for a personal work-tracking system.
...
def classify_messages(company, channel, messages, timeout=120):
    ...
    prompt = PROMPT_TEMPLATE.format(company=company, channel=channel, messages=formatted)
    ...
    return json.loads(_extract_json_array(result.stdout.strip()))
